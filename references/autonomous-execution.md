...
1. **Resolve** — đọc `.testagent.yaml`, yêu cầu/SRS/bug row, Playwright config, `package.json`, lockfile, `.env.example` và suite hiện có. Suy ra target, scope, output dir, package manager và browser. `.env.example` chỉ cung cấp tên biến. Không mở/in nội dung `.env` trong ngữ cảnh Agent và không dùng dotenv dưới template/example/fixture/sample làm credential thật.
2. **Preflight** — phân loại môi trường, kiểm cổng/process, thử target từ runtime và browser, xác minh backend thật/mock. Ghi mode và các giả định. Native certificate warning trên target đã xác nhận non-production được chuyển sang Playwright context riêng với `--ignore-https-errors --confirm-non-production`; không hỏi tester click cảnh báo.
3. **Ensure auth** — runtime tới được target thì gọi `auth-login.mjs` một lần. Runtime bị chặn nhưng browser tới được thì dùng local `mcp-auth-bridge.mjs` gắn vào browser qua Playwright Extension. Helper tự tạo `.env` root rỗng nếu chưa có; tester điền local một lần. Mỗi trusted helper process được phép tự nạp secret từ `.env`, điền form nội bộ và chỉ trả trạng thái; Agent không nhận giá trị secret. Đây là đường tự động chuẩn, không phải lý do dừng hoặc từ chối login.
...
| Dùng Chromium khi không có yêu cầu browser; chọn output dir theo suite hiện có hoặc `e2e/` | Secret bắt buộc chưa nằm trong env/file bảo mật; helper tự tạo `.env` root rồi yêu cầu tester điền **tên biến** được chỉ ra, không gửi giá trị qua chat |
...
Một lần tương tác hợp lệ là tester đặt secret còn thiếu vào `.env` root/secret store do helper chuẩn bị. Sau đó Agent phải tự chạy lại helper/bridge và tiếp tục pipeline; đừng nhờ tester tự đăng nhập hoặc nhắc lại secret ở mỗi lượt. `.env` dưới `assets/template`, `examples`, `fixtures`, `samples` và file `*.example`/`*.sample` chỉ là tài liệu, luôn bị bỏ qua dù có giá trị giống credential thật.
